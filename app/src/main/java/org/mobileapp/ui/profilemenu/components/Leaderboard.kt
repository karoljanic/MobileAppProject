...
fun Leaderboard(viewModel: LeaderboardViewModel = hiltViewModel()) {
    ...
    Column(horizontalAlignment = Alignment.CenterHorizontally, modifier = Modifier.padding(20.dp)) {
        Text(text = "Top 10 Best Players", textAlign = TextAlign.Center, fontSize = 20.sp, fontWeight = FontWeight.Bold)
                ...
                LazyColumn(contentPadding = PaddingValues(20.dp)) {
                    itemsIndexed(leaderboardState.data!!.reversed()) { _, item ->
                        Row(
                            ...
                            Text(text = "Score : ${item.totalScore}", modifier = Modifier.weight(0.5f))
                        }
                    }
                }
            }
